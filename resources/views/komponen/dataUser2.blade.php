...
">
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <div class="p-4 md:p-5">
                        <div>
                            <form action="{{route('export.templateUser')}}">
                            <label class="block mb-2 text-sm font-medium text-gray-900"
                                for="file_input">Download file template excel</label>
                                    <button type="submit" class="block text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5">
                                        <i class="fas fa-download fa-3x"></i> <!-- Ikon Download -->
                                    </button>
                ...
                @foreach ($users as $dataUser)
                    <tr
                        ...
                        <td class="px-6 py-4 flex justify-center gap-2">
                            ...
                            <button data-modal-target="popup-modal-hapus{{ $dataUser->id }}" data-modal-toggle="popup-modal-hapus{{ $dataUser->id }}" type="button">
                                <i class="fas fa-trash-alt"></i> <!-- Font Awesome 5 -->
                            </button>
                            {{-- modal konfirmasi hapus user --}}
                            <div id="popup-modal-hapus{{ $dataUser->id }}" tabindex="-1"
                                class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow">
                                        <button type="button"
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            <form action="{{ route('delete-user',$dataUser->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
...
{{-- popup jika berhasil tambah tanggal dan hapus --}}
@if (session('success'))
